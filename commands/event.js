...
module.exports.run = async (bot, message, args) => { 
  ...
  if (!message.channel.name.includes("events")) return message.channel.send("La commande `!event` est uniquement disponible dans le channel `#event`.");
  // Exit if the command doesn't contain argument(s) > '!event args'
  if (args.length === 0) return message.channel.send("Pour créer un **event**, il suffit d'effectuer la commande `!event <date> <sujet> <description>`.\n• La `<date>` doit **impérativement** être au format `jj/mm` (**jour**\/**mois**).\n• Le `<sujet>` peut être le nom de l'evenement. ex: *Cinéma*, *Voyage*, *Souper*, *Congé* ... Celui-ci doit **impérativement** être en **un seul** mot.");
  // Check if the command is called correctly
  if (args.length < 3) return message.channel.send("La commande `!event` n'a pas été effectuée correctement.");
